...
from app.themovie.helpers.dynamic_summary_config import analyze_document_for_summary
...
@router.post("/summary/url", response_model=dict)
async def summarize_url(request: URLSummaryRequest):
    """
    Tóm tắt nội dung từ URL
    
    Args:
        request: URLSummaryRequest object containing URL and parameters
        
    Returns:
        JSON response with URL content summary
    """
    try:
        # Validate URL
        if not request.url or not request.url.startswith(('http://', 'https://')):
            raise HTTPException(
                status_code=400,
                detail="URL không hợp lệ. URL phải bắt đầu với http:// hoặc https://"
            )
        ...
        extracted_text = await text_service.extract_text_from_url(request.url)
        
        # Validate extracted text
        if len(extracted_text.strip()) < 100:
            raise HTTPException(
                status_code=400,
                detail="Không thể trích xuất đủ nội dung từ URL để tóm tắt"
            )
        ...
        return JSONResponse(
            ...
            content={
                "status": ResponseStatus.SUCCESS,
                "data": summary_result,
                "message": "Tóm tắt nội dung từ URL thành công"
            }
        )
        ...
        logger.error(f"Error in URL summarization: {str(e)}")
        return JSONResponse(
            status_code=500,
            content={
                "status": ResponseStatus.ERROR,
                "message": f"Lỗi khi tóm tắt từ URL: {str(e)}"
            }
        )
...
async def health_check():
    ...
    try:
        ...
        ai_services_status = {
            "openai": text_service.openai_service is not None,
